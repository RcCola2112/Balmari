...
if (empty($title)) {
    ...
    try {
        ...
        $base_upload_dir = '../assets/projects/progress/';
            ...
            if (!in_array($ext, $allowed_types)) {
                ...
                if (!move_uploaded_file($cover_file['tmp_name'], $cover_path)) {
                    ...
                    if (file_put_contents($metadata_path, json_encode($metadata, JSON_PRETTY_PRINT))) {
                        $success_msg = 'Project uploaded successfully with ' . $detail_count . ' progress image(s)!';
                        if (!empty($detail_errors)) {
                            $success_msg .= ' (' . count($detail_errors) . ' image(s) skipped due to errors)';
                        }
                        $_SESSION['upload_success'] = $success_msg;
                        header('Location: upload_progress.php?success=1');
                        exit();
                    } else {
...
if (!empty($error_msg)) {
    ...
    header('Location: upload_progress.php?error=1');
    exit();
}
?>
