...
$page_title = "Completed Work";
require_once 'includes/header.php';

// Load completed projects from filesystem
$projects_dir = __DIR__ . '/../assets/projects/completed/';
$completed_projects = [];
    ...
    foreach ($project_folders as $folder) {
        ...
        if (file_exists($metadata_file)) {
            ...
            if ($metadata) {
                ...
                $metadata['cover_image_url'] = '../assets/projects/completed/' . $folder . '/' . ($metadata['cover_image'] ?? '');
                $metadata['project_path'] = $project_path;
                $completed_projects[] = $metadata;
            }
        }
    }
}
...
if (isset($_POST['delete_project']) && isset($_POST['project_id'])) {
    ...
    if (is_dir($project_path)) {
        ...
        header('Location: manage_completed.php?deleted=1');
        exit();
    }
}
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-4xl font-['Playfair_Display'] font-semibold text-[#FFFFFF]">Completed Work</h1>
        <button onclick="openAddModal()" class="bg-[#5C8374] text-[#183D3D] px-6 py-3 rounded font-semibold hover:bg-[#44444E] hover:text-[#FFFFFF] transition">+ Add New Project</button>
    </div>
    ...
    <?php if (empty($completed_projects)): ?>
        <div class="text-center py-20">
            <p class="text-[#D3DAD9] text-lg mb-6">No completed projects yet</p>
            <button onclick="openAddModal()" class="inline-block bg-[#5C8374] text-[#183D3D] px-6 py-3 rounded font-semibold hover:bg-[#44444E] hover:text-[#FFFFFF] transition">Upload Your First Project</button>
        </div>
    <?php else: ?>
                ...
                <div class="bg-[#183D3D] rounded-lg shadow-lg overflow-hidden border border-[#44444E] hover:border-[#5C8374] transition">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex gap-2">
                            <a href="../project_details.php?id=<?php echo urlencode($project['project_id']); ?>&type=completed" class="flex-1 bg-[#5C8374] text-[#183D3D] px-3 py-2 rounded text-sm font-medium hover:bg-[#44444E] hover:text-[#FFFFFF] transition text-center">View</a>
                            <a href="edit_project.php?id=<?php echo urlencode($project['project_id']); ?>&type=completed" class="flex-1 bg-[#44444E] text-[#FFFFFF] px-3 py-2 rounded text-sm font-medium hover:bg-[#5C8374] hover:text-[#183D3D] transition text-center">Edit</a>
...
<div id="addModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-[#183D3D] rounded-xl shadow-2xl border border-[#44444E] max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-6 text-[#D3DAD9]">
            <form action="upload_completed_process.php" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input 
                        ...
                        required
                    >
                    <p class="text-[#5C8374]/70 text-xs mt-1">Featured photo</p>
                </div>
                ...
                <div>
                    ...
                    <input 
                        ...
                        class="w-full text-[#D3DAD9] text-sm"
                    >
                    <p class="text-[#5C8374]/70 text-xs mt-1">Project gallery images</p>
                </div>
